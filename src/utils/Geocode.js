...
export default {
	 ...
	 * @returns {Promise}
	 */
	async fromCity(city, apiKey, language, region) {
		if (!city) {
			log('Provided city is invalid', true);
			return Promise.reject(new Error('Provided city is invalid'));
		}
		const components = `country:US|locality:${city}`;
		let url = `${GOOGLE_API}?components=${encodeURIComponent(components)}`;
		if (apiKey || API_KEY) {
			API_KEY = apiKey || API_KEY;
			url += `&key=${API_KEY}`;
		}
